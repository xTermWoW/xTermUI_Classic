...
local vmajor, vminor = "LibGratuity-3.0", 90000 + tonumber(("$Revision: 40 $"):match("%d+"))
...
function lib:CreateTooltip()
	local basename="LibGratuity"
	local tt
	for i=1,10 do
		if not _G[basename.."Tooltip"..i] then
			tt = CreateFrame("GameTooltip",basename.."Tooltip"..i,UIParent,"GameTooltipTemplate")
			break
		end
	end
	if not tt then error("Couldn't create a new tooltip.") end
...
--	tooltip:SetParent()
	...
	for i=1,30 do
		tt:AddDoubleLine(i)
		local left,right = _G[ttname.."TextLeft"..i],_G[ttname.."TextRight"..i]
		if not left or not right then print("WTF? no "..basename.."TextLeft"..i.." or Right") break end -- what the hell?
		self.vars.Llines[i], self.vars.Rlines[i] = left,right
...
function lib:CreateSetMethods()
	for _,m in pairs(methods) do
		...
		self[meth] = function(self, ...)
			...
			--num=num+1 ZGV:Debug("%d %s %s",num,meth,...)
			--return GRAT_SAFE or self.vars.tooltip[meth](self.vars.tooltip,...) --unsafe now
