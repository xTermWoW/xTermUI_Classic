...
function ScrollTable:New(parent,name,COLUMNS,DATA,useparent)
	...
	for k,col in ipairs(COLUMNS) do
		...
		if col.type=="button" or col.type=="headerbutton" or col.sortable then 
			...
			fs:SetJustifyH(col.titlej or col.textj)
	...
	for n=1,DATA.ROW_COUNT do
		frame.rows[n] = CHAIN(ui:Create("Button",frame,name.."_ROW"..n))
		...
		for k,col in ipairs(COLUMNS) do
			...
			elseif col.type=="button" then 
				...
				widget:GetFontString():SetJustifyH(col.textj)
			elseif col.type=="toggle" then
		...
		row.hilite = CHAIN(row:CreateTexture(row:GetName().."Hilite","HIGHLIGHT")) :SetAllPoints() :SetColorTexture(1,1,1,DATA.ROWHIGHLIGHT and 0.15 or 0) .__END
		row.back = CHAIN(row:CreateTexture(row:GetName().."Back","ARTWORK")) :SetAllPoints() :SetColorTexture(1,1,1,DATA.ROWBACKGROUND and 1 or 0) .__END
