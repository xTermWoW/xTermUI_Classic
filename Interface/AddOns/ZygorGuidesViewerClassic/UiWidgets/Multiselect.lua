...
function Multiselect:New(parent,style,frameLevel)
	...
	local multiselectObj = CHAIN(AceGUI:Create("Dropdown-Z"))
	...
	if style == 1 then
		ZGV.F.AssignButtonTexture(multiselectButton,SkinData("TitleButtons"),11,32)
	elseif style == 2 then
		-- Gets a texture then a gray backdrop
		ZGV.F.AssignButtonTexture(multiselectButton,SkinData("TitleButtons"),19,32)
...
function Multiselect:AddItem(obj,value,dbname,callback, tooltip)
	...
	item.check = CHAIN(item.zygorbackdrop:CreateTexture("OVERLAY"))
		:SetColorTexture(unpack(SkinData("AccentColor")))
		:SetAllPoints()
		:Hide()
	.__END
